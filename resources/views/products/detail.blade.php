                                ...
                                <div class="row">
                                    ...
                                    <div class="col-12 mt-4">
                                        <div class="row d-flex justify-content-between">
                                            <div class="col-6 ">
                                                <div
                                                    class="d-flex justify-content-center align-items-center review-box">
...
async function getVendorDetails(vendorID) {
        ...
        taxSetting.get().then(async function(taxSettingSnapshots) {
            try {
                ...
                if (getCookie('service_type') == "Ecommerce Service") {
                    var section_delivery_charge = taxArray.delivery_charge ? parseFloat(taxArray
                        .delivery_charge) : 0;
    ...
    vendorDetailsRef.get().then(async function(vendorSnapshots) {
        ...
        $("#seller-image").html('<a href="' + view_vendor_details +
            '"><img style="height: 65px; width: 65px; border-radius: 50%" src="' + photo + '"></a>');
                    ...
                    if (specialOfferVendor[i]['day'] == currentDay) {
                        if (specialOfferVendor[i]['timeslot'].length > 0) {
                            for (j = 0; j < specialOfferVendor[i]['timeslot'].length; j++) {
                                if (currentTime >= specialOfferVendor[i]['timeslot'][j]['from'] &&
                                    currentTime <= specialOfferVendor[i]['timeslot'][j]['to']) {
...
function getVendorProducts(vendorID) {
    database.collection('vendor_products').where("vendorID", "==", vendorID).where("publish", "==", true).get().then(
        async function(snapshots) {
            var total = snapshots.docs.length;
            $(".vendor-total-product").text(total);
        });
    database.collection('vendor_products').where("vendorID", "==", vendorID).where("publish", "==", true).where('id',
        "!=", id).limit(5).get().then(async function(snapshots) {
        var html = '';
...
function buildVendorProductsHTML(snapshots) {
    ...
    alldata.forEach((listval) => {
        ...
        var rating = 0;
        var reviewsCount = 0;
        if (val.hasOwnProperty('reviewsSum') && val.reviewsSum != 0 && val.hasOwnProperty('reviewsCount') && val
            .reviewsCount != 0) {
...
function getUsersReviews(vendorProduct, limit) {
    ...
    if (vendorProduct.product_specification != null && vendorProduct.product_specification != "" && !$.isEmptyObject(
            vendorProduct.product_specification)) {
        var html = '';
...
function buildRatingsAndReviewsHTML(vendorProduct, reviewsSnapshots) {
    ...
    var rating = 0;
    var reviewsCount = 0;
    if (vendorProduct.hasOwnProperty('reviewsSum') && vendorProduct.reviewsSum != 0 && vendorProduct.hasOwnProperty(
            'reviewsCount') && vendorProduct.reviewsCount != 0) {
    ...
    allreviewdata.forEach((listval) => {
        ...
        if (val.profile == '' || val.profile.indexOf('firebasestorage.googleapis.com') == -1) {
            reviewhtml = reviewhtml + '<a href="javascript:void(0);"><img alt="#" src="' + placeholderImageSrc +
                '" class="mr-3 rounded-pill"></a>';
        } else {
            try {
                reviewhtml = reviewhtml + '<a href="javascript:void(0);"><img alt="#" src="' + val.profile +
                    '" class="mr-3 rounded-pill"></a>';
            } catch (err) {
        ...
        reviewhtml = reviewhtml + '</div></div><div class="reviews-members-body w-100"><p class="mb-2">' + val
            .comment + '</p>' + photos + '</div></div>';
    });
    reviewhtml += '</div>';
...
function checkFavoriteProduct(productID) {
    ...
    database.collection('favorite_item').where('product_id', '==', productID).where('user_id', '==', user_id).get()
        .then(async function(favoriteItemsnapshots) {
...
$(document).on("click", "a[name='addToFavorite']", function(e) {
    ...
    database.collection('favorite_item').where('product_id', '==', product_id).where('user_id', '==', user_id)
...
function buildHTML(snapshots) {
    ...
    if (vendorProduct.item_attribute != null && vendorProduct.item_attribute != "" && vendorProduct.item_attribute
        .attributes.length > 0 && vendorProduct.item_attribute.variants.length > 0) {
        ...
        if (vendorProduct.hasOwnProperty('quantity')) {
            if (vendorProduct.quantity == -1) {
                ...
                html = html + '<span id="variant_qty">{{trans("lang.qty_left")}}: ' + vendorProduct.quantity +
                '</span>';
            }
        }
    }
    ...
    if (vendorProduct.item_attribute != null && vendorProduct.item_attribute != "" && vendorProduct.item_attribute
        ...
        html = html + '<div class="attribute_list" id="attribute-list-' + vendorProduct.id + '" data-pid="' +
            vendorProduct.id + '" data-product="' + btoa(JSON.stringify(vendorProduct)) + '"></div>';
        html = html + '</div>';
        html = html + '</div>';
    ...
    if (vendorProduct.hasOwnProperty('addOnsPrice') && vendorProduct.addOnsPrice.length > 0) {
        ...
        vendorProduct.addOnsPrice.forEach(async (product_price) => {
            ...
            html += '<input data-price="' + product_price + '" type="checkbox" id="' + vendorProduct.id +
                '_extra_' + total + '" name="extra_' + vendorProduct.id + '" value="' + vendorProduct
                .addOnsTitle[total] + '" class="custom-control-input extra_' + vendorProduct.id + '">';
            html += '<label class="custom-control-label" for="' + vendorProduct.id + '_extra_' + total +
                '">' + vendorProduct.addOnsTitle[total] + ' <span class="">+$' + product_price +
                '</span></label>';
            html += '</div>';
            html += '</div>';
            total++;
    ...
    html += '<input type="hidden" name="name_' + vendorProduct.id + '" id="name_' + vendorProduct.id + '" value="' +
        vendorProduct.name + '">';
    html += '<input type="hidden" id="price_' + vendorProduct.id + '" name="price_' + vendorProduct.id + '" value="' +
        price + '">';
    html += '<input type="hidden" id="dis_price_' + vendorProduct.id + '" name="dis_price_' + vendorProduct.id +
        '" value="' + vendorProduct.disPrice + '">';
    html += '<input type="hidden" id="quantity_' + vendorProduct.id + '" name="quantity_' + vendorProduct.id +
        '" value="' + vendorProduct.quantity + '">';
    html += '<input type="hidden" id="image_' + vendorProduct.id + '" name="image_' + vendorProduct.id + '" value="' +
        vendorProduct.photo + '">';
    html += '<input type="hidden" id="veg_' + vendorProduct.id + '" name="veg_' + vendorProduct.id + '" value="' +
        vendorProduct.veg + '">';
    html += '<input type="hidden" id="category_id_' + vendorProduct.id + '" name="category_id_' + vendorProduct.id +
        '" value="' + vendorProduct.categoryID + '">';
    ...
    if (variation_sku) {
        ...
        if (variant_info.length > 0) {
            ...
            if (variant_quantity == '-1') {
                ...
                $('#variation_info_' + vendorProduct.id).find('#variant_qty').html('{{trans("lang.qty_left")}}: ' +
                    variant_quantity);
            }
        }
    }
...
function getAttributeHtml(vendorProduct, attribute) {
    ...
    attributeHtmlRes = vendorAttributesRef.get().then(async function(attributeRef) {
        ...
        $.each(attribute.attribute_options, function(i, option) {
            ...
            html += '<input type="radio" id="attribute-' + attribute.attribute_id + '-' + option +
                '" name="attribute-options-' + attribute.attribute_id + '" value="' + option +
                '" ' + ischecked + ' class="custom-control-input">';
            html += '<label class="custom-control-label" for="attribute-' + attribute.attribute_id +
                '-' + option + '">' + option + '</label>';
            html += '</div>';
        });
        html += '</div>';
