...
<div class="siddhi-home-page">
    ...
    var itemCategoriesref = database.collection('vendor_categories').where('section_id', '==', section_id).where(
        "publish", "==", true).limit(7);
    ...
    bannerref.get().then(async function(banners) {
        ...
        if (position1_banners.length > 0) {
            ...
            for (banner of position1_banners) {
                ...
                html +=
                    '</div><div class="banner-text"><h3>{{trans("lang.we_give_best_product")}} {{trans("lang.for_you")}}</h3><p>{{trans("lang.we_give_best_product")}} {{trans("lang.for_you")}}</p><div class="ban-btn"><a href="#" class="btn btn-primary">{{trans("lang.order_now")}}</a></div></div>';
                html += '</div>';
            }
    ...
    const popularRestauantRef = geoFirestore.collection('vendors').where('section_id', '==', section_id).where(
        'reviewsSum', '>', 4);
    ...
    var couponsRef = database.collection('coupons').where('isEnabled', '==', true).where("section_id", "==", section_id)
        .orderBy("expiresAt").startAt(new Date()).limit(4);
    ...
    function buildHTMLItemCategory(foodCategories) {
        ...
        alldata.forEach((listval) => {
            ...
            if (val.publish) {
                ...
                html = html +
                    '<div class="col-md-2 top-cat-list"><a class="d-block text-center cat-link" href="' +
    ...
    function buildHTMLHomeCategoryProducts(category_id) {
        ...
        var vendorCatRef = database.collection('vendor_products').where('categoryID', "==", category_id).where(
            'section_id', '==', section_id).limit(4);
        ...
        var productHtmlRes = vendorCatRef.get().then(async function(nearestRestauantSnapshot) {
            ...
            alldata.forEach((listval) => {
                ...
                html = html +
                    '<h6 class="mb-1 popular_food_category_ pro-cat" id="popular_food_category_' +
    ...
    function buildHTMLMostSaleStore(mostSaleSnapshot) {
        ...
        alldata.forEach((listval) => {
            ...
            html = html + '<a href="' + view_vendor_details + '"><img alt="#" src="' + photo +
                '" class="img-fluid item-img w-100"></a></div>';
            html = html + '</div>';
            html = html + '</div></div></div>';
        });
    ...
    function buildHTMLPopularItem(popularItemsnapshot) {
        ...
        alldata.forEach((listval) => {
            ...
            html = html + '<a href="' + view_vendor_details + '"><img alt="#" src="' + photo +
                ...
                view_vendor_details + '" class="text-black">' + val.name + '</a></h6>';
            var popularItemCategorytitle = popularItemCategory(val.categoryID, val.id);
            html = html + '<h6 class="text-gray mb-1 cat-title" id="popular_food_category_' + val.categoryID +
                '_' + val.id + '"></h6>';
            ...
            if (val.hasOwnProperty('disPrice') && val.disPrice != '' && val.disPrice != '0') {
                ...
                html = html + '<h6 class="text-gray mb-1 price">' + dis_price + '  <s>' + or_price +
                '</s></h6>';
