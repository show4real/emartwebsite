        ...
        <section class="all-stores">
            ...
            <div class="container">
                ...
                <div class="row fu-loadmore-btn">
                    <a class="page-link loadmore-btn" href="{{ route('vendors')}}" data-dt-idx="0"
                        tabindex="0">{{trans('lang.see_all_store')}}</a>
                </div>
...
var itemCategoriesref = database.collection('vendor_categories').where('section_id', '==', section_id).where("publish",
    "==", true).limit(7);
var bannerref = database.collection('banner_items').where('sectionId', '==', section_id).where("is_publish", "==", true)
    .orderBy('set_order', 'asc');
var vendorsref = geoFirestore.collection('vendors').where('section_id', '==', section_id);
var refCurrency = database.collection('currencies').where('isActive', '==', true);
var brandsRef = database.collection('brands').where('sectionId', '==', section_id).where('is_publish', '==', true)
    .limit(7);
...
const popularRestauantRef = geoFirestore.collection('vendors').where('section_id', '==', section_id).where('reviewsSum',
    '>', 4);
...
var couponsRef = database.collection('coupons').where('isEnabled', '==', true).where("section_id", "==", section_id)
...
if (address_lat == '' || address_lng == '' || address_lng == NaN || address_lat == NaN || address_lat == null ||
    address_lng == null) {
...
async function getHomepageCategory() {
    ...
    var home_cat_ref = database.collection('vendor_categories').where('section_id', '==', section_id).where(
        "publish", "==", true).where('show_in_homepage', '==', true).limit(5);
    ...
    home_cat_ref.get().then(async function(homeCategories) {
        ...
        if (homeCategorieshtml != "" && position2_banners.length > 0) {
            ...
            $(".brands_item").after(
                '<section class="middle-banners"><div class="container"><div class="middle_banner" id="middle_banner1">' +
    ...
    if (alldata.length > 0) {
        ...
        alldata.forEach((listval) => {
            ...
            html = html + '<a href="' + view_vendor_details + '"><img alt="#" src="' + photo +
                ...
                view_vendor_details + '" class="text-black vendor_title_' + vendor_id_single + '"></a></h6>';

            //html = html + '<p class="text-gray mb-1 small location vendor_location_' + vendor_id_single + '"></p>';
            html = html +
                '<div class="text-gray mb-1 small"><a href="javascript:void(0)" onclick="copyToClipboard(`' +
...
async function getMostSalesStore() {
    var mostSalesStore = database.collection('vendors').where('section_id', '==', section_id).orderBy('createdAt',
        'desc').limit(4);
...
function buildAllStoresHTML(snapshots) {
    ...
    alldata.forEach((listval) => {
        ...
        html = html + '<a href="' + view_vendor_details + '"><img alt="#" src="' + photo +
            '" class="img-fluid item-img w-100"></a></div><div class="py-2 position-relative"><div class="list-card-body">';

        html = html +
            '<div class="star position-relative"><span class="badge badge-success "><i class="feather-star"></i>' +
            rating + ' (' + reviewsCount + ')</span></div>';

        html = html + '<h6 class="mb-1 popul-title"><a href="' + view_vendor_details + '" class="text-black">' +
            val.title + '</a></h6><h6>' + val.location + '</h6><h6 class="pr-discount vendor_dis_' + val.id +
            '"></h6>';
...
function buildBrandsHTML(snapshots) {
    ...
    alldata.forEach((listval) => {
        ...
        html = html +
            '<div class="col-md-2 brand-list"><div class="list-card position-relative"><div class="list-card-image">';
        ...
        html = html + '<a href="' + view_vendor_details + '"><img alt="#" src="' + photo +
            '" class="img-fluid item-img w-100"></a></div><div class="p-2 position-relative brand-title"><div class="list-card-body"><h6><a href="' +
            view_vendor_details + '" class="text-black mb-0">' + val.title + '</a></h6>';
...
function buildHTMLItemCategory(foodCategories) {
    ...
    alldata.forEach((listval) => {
        ...
        html = html + '<div class="col-md-2 top-cat-list"><a class="d-block text-center cat-link" href="' +
            trending_route + '"><span class="cat-img"><img alt="#" src="' + photo +
...
function buildHTMLHomeCategoryProducts(category_id) {
    ...
    var productHtmlRes = vendorCatRef.get().then(async function(nearestRestauantSnapshot) {
        ...
        alldata.forEach((listval) => {
            ...
            html = html +
                '<h6 class="mb-1 popular_food_category_ pro-cat" id="popular_food_category_' + val
            ...
            if (val.hasOwnProperty('disPrice') && val.disPrice != '' && val.disPrice != '0') {
                ...
                html = html + '<span class="pro-price">' + dis_price + '  <s>' + or_price +
                    '</s></span>';
            } else {
                var or_price = '';
                if (currencyAtRight) {
...
function buildHTMLPopularStore(popularRestauantSnapshot) {
    ...
    alldata.forEach((listval) => {
        ...
        html = html + '<a href="' + view_vendor_details + '"><img alt="#" src="' + photo +
            ...
            '</h6><h6 class="pr-discount vendor_dis_' + val.id + '"></h6>';
...
function buildHTMLNewProducts(newproductSnapshot) {
    ...
    alldata.forEach((listval) => {
        ...
        html = html + '<a href="' + view_product_details + '"><img alt="#" src="' + photo +
...
function buildHTMLMostSaleStore(mostSaleSnapshot) {
    ...
    alldata.forEach((listval) => {
        ...
        html = html +
            '<div class="col-md-3"><div class="align-items-center list-card position-relative"><div class="list-card-image">';
        ...
        html = html + '<a href="' + view_vendor_details + '"><img alt="#" src="' + photo +
            '" class="img-fluid item-img w-100"></a></div><div class="most-pop-bottom"><div class="list-card-body"><h6 class="mb-1"><a href="' +
            view_vendor_details + '" class="arv-title">' + val.title +
            '</a></h6><h6 class="arv-discount vendor_dis_' + val.id + '"></h6>';
        html = html + '</div>';
        html = html + '</div></div></div>';
    });
    html = html + '</div>';
...
function buildHTMLPopularItem(popularItemsnapshot) {
    ...
    alldata.forEach((listval) => {
        ...
        html = html + '<a href="' + view_vendor_details + '"><img alt="#" src="' + photo +
            '" class="img-fluid item-img w-100"></a></div><div class="py-2 position-relative"><div class="list-card-body"><h6 class="product-title mb-1"><a href="' +
            view_vendor_details + '" class="text-black">' + val.name + '</a></h6>';
        var popularItemCategorytitle = popularItemCategory(val.categoryID, val.id);
...
async function catHaveProducts(categoryId) {
    var response = database.collection('vendor_products').where("categoryID", "==", categoryId).get().then(function(
        CatProducts) {
